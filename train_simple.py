...
if __name__ == '__main__':
    ...
    parser.add_argument('--color_jitter', action='store_true', help='use color jitter in training')    # 在训练中使用颜色抖动
    parser.add_argument('--batchsize', default=32, type=int, help='batchsize')
    parser.add_argument('--stride', default=2, type=int, help='stride')
    parser.add_argument('--erasing_p', default=0, type=float, help='Random Erasing probability, in [0,1]')    # 随机擦除概率
    ...
    class_names = image_datasets['train'].classes                             # 种类

    use_gpu = torch.cuda.is_available()    # gpu是否可用

    since = time.time()
    inputs, classes = next(iter(dataloaders['train']))    # 不断取输入数据和类别
    ...
    def train_model(model, criterion, optimizer, scheduler, num_epochs=25):
        ...
        for epoch in range(num_epochs):
            ...
            for phase in ['train', 'val']:
                ...
                # Iterate over data.
                for data in dataloaders[phase]:  # 轮询取数，训练模型
                    # get the inputs
                    inputs, labels = data
                ...
                if phase == 'val':
                    ...
                    if epoch % 10 == 9:  # 每10轮，保存一次
                        save_network(model, epoch)
                    draw_curve(epoch)  # 每轮都更新loss曲线
    ...
    ######################################################################
    # Finetuning the conv net，微调神经网络
    # ----------------------
    #
    # Load a pretrainied model and reset final fully connected layer.，加载预训练模型，并重置最终模型
    #
